...
bad_block:	kr_error(inode->i_sb, "ext2_xattr_get",
...
ext2_xattr_list(struct dentry *dentry, char *buffer, size_t buffer_size)
{
	...
	if (HDR(bh)->h_magic != cpu_to_le32(EXT2_XATTR_MAGIC) ||
	    HDR(bh)->h_blocks != cpu_to_le32(1)) {
bad_block:	kr_error(inode->i_sb, "ext2_xattr_list",
			"inode %ld: bad block %d", inode->i_ino,
			EXT2_I(inode)->i_file_acl);
		error = -EIO;
		goto cleanup;
...
bad_block:		kr_error(sb, "ext2_xattr_set",
				"inode %ld: bad block %d", inode->i_ino, 
				   EXT2_I(inode)->i_file_acl);
			error = -EIO;
			goto cleanup;
...
ext2_xattr_delete_inode(struct inode *inode)
{
	...
	if (!ext2_data_block_valid(sbi, EXT2_I(inode)->i_file_acl, 0)) {
		kr_error(inode->i_sb, "ext2_xattr_delete_inode",
...
again:
	...
	while (ce) {
		...
		if (!bh) {
			kr_error(inode->i_sb, "ext2_xattr_cache_find",
